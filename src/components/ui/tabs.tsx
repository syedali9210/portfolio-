...
const TabsList = forwardRef<HTMLDivElement, TabsListProps>(
  ({ children, className, petBuddy = false, ...props }, ref) => {
    ...
    useEffect(() => {
      // Syncs the optimistic index to the resolved selected value (e.g. on
      // programmatic/controlled value changes, not just clicks).
          ...
          <AnimatePresence>
            {hoverRect && !isHoveringSelected && selectedRect && (
              <motion.div
                ...
                exit={
                  // AnimatePresence computes `exit` once at unmount time (not
                  // every render), so reading the ref here is safe in practice.
